...
Sub JoinGame(ByVal index As Long)
    ...
    If GetPlayerAccess(index) <= ADMIN_MONITOR Then
        Call GlobalMsg(GetPlayerName(index) & " has come to play on " & Options.Game_Name & "!", JoinLeftColor)
    Else
        Call GlobalMsg(GetPlayerName(index) & " has come to play on " & Options.Game_Name & "!", White)
    End If
    ...
    With Map(GetPlayerMap(index)).Tile(GetPlayerX(index), GetPlayerY(index))
        ...
        If .Type = TILE_TYPE_TRAP Then
            ...
            If GetPlayerVital(index, HP) - amount <= 0 Then
                KillPlayer index
                PlayerMsg index, "You stepped on a trap and got knocked out.", BrightRed
            Else
                SetPlayerVital index, HP, GetPlayerVital(index, HP) - amount
                PlayerMsg index, "You got hurt from a hidden trap!", BrightRed
